...
namespace torc::models {
    ...
    void Centroid::DynamicsDerivative(const vectorx_t &state,
      ...
      for (int i = 0; i < n_contacts_; i++) {
        ...
        J_frame =
            getFrameJacobian(pin_model_, *pin_data_, contact_frames_idxs_.at(i),
                             pinocchio::WORLD);
        const matrixx_t dr_dq = J_frame.topRows(LINEAR_DIM) -
                                Jcom; // oh no this has nv not nq columns
      ...
      B.bottomRightCorner(n_actuated_, n_actuated_) =
          matrixx_t::Identity(n_actuated_, n_actuated_);
      B.block(COM_DOF, n_contacts_ * FORCE_DIM, BASE_DOF, n_actuated_) =
          -A_b_inv * A_j;
      // iterate through the forces
      for (int i = 0; i < n_contacts_; i++) {
        const matrixx_t iden3 = matrixx_t::Identity(LINEAR_DIM, FORCE_DIM);
        B.block(0, i * FORCE_DIM, LINEAR_DIM, FORCE_DIM) = iden3; // linear
        vector3_t r = pin_data_->oMi[contact_frames_idxs_.at(i)].translation() -
                      com_pos; // rotational
    ...
    vectorx_t Centroid::BuildState(const vectorx_t &hcom, const vectorx_t &q) {
